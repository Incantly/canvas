...
title: "tldraw vs Excalidraw vs Incantly Canvas: choosing a whiteboard library"
...
**tldraw**, **Excalidraw**, and **Incantly Canvas**. All three will put a
competent infinite canvas in your app. They differ on licensing, weight, and
which stacks they serve. (Disclosure: this is Incantly Canvas's blog — so we
...
**Incantly Canvas** is MIT too. It shows a small "Made with Incantly Canvas" badge in
...
the comparison is between Excalidraw and Incantly Canvas.

## Framework support

| | tldraw | Excalidraw | Incantly Canvas |
|---|---|---|---|
| React | Yes | Yes | Yes |
| Plain JS / other frameworks | No | No | **Yes** |
| React Native | No | No | **Yes** |

tldraw and Excalidraw are React components at their core. Incantly Canvas's
...
Incantly Canvas is measured the same way.

| | Bundle (min + gzip) | Runtime dependencies |
|---|---|---|
| tldraw 5.2.5 | 496 kB | 14 |
| Excalidraw 0.18.1 | 344 kB | 31 |
| **Incantly Canvas 0.1.3** | **25 kB** | **0** |

So the Incantly Canvas core is roughly 20× lighter than tldraw and 14× lighter
than Excalidraw. For scale in the other direction: the entire Incantly Canvas
...
Add ~2 kB gzipped for `canvas.css` if you ship the default styling, and
...
localization). Incantly Canvas ships a complete but focused whiteboard; its
[roadmap](https://github.com/Incantly/canvas/issues/1) is public.
...
Incantly Canvas sits between: crisp UI chrome, hand-drawn wobble in the shapes,
...
- **Incantly Canvas** builds sync into the data model — every change emits a
...
of production hardening, and large test surfaces. Incantly Canvas is new —
smaller API surface, ~120 tests, one engine shared across three packages,
and an open [issue tracker](https://github.com/Incantly/canvas/issues)
...
- **Want MIT + a lightweight embed, plain-JS/Vue/Svelte support, or React
  Native:** [Incantly Canvas](https://github.com/Incantly/canvas) — `npm install
  @incantly/canvas-react` and you'll know in five minutes.

Corrections welcome — licensing and features change, and we'd rather this
page be right than flattering:
[open an issue](https://github.com/Incantly/canvas/issues).
