...
describe('bounds', () => {
  ...
  it('ptsBounds respects stride 3', () => {
...
describe('points', () => {
  ...
  it('distToPolyline walks all segments', () => {
  ...
  it('pointInPolygon / pointInEllipse', () => {
    ...
    expect(pointInEllipse(1, 1, 0, 0, 0, 3)).toBe(false)
  })

  it('segIntersectsBounds catches crossing and contained segments', () => {
    const r = { x: 0, y: 0, w: 10, h: 10 }
    expect(segIntersectsBounds(-5, 5, 15, 5, r)).toBe(true)
    expect(segIntersectsBounds(2, 2, 8, 8, r)).toBe(true)
    expect(segIntersectsBounds(-5, -5, -1, 20, r)).toBe(false)
  })
})
...
describe('geo polygon builders', () => {
  it('builds each kind with the right vertex count', () => {
    ...
    expect(geoPolygon('unknown', 10, 10).length).toBe(8)
  })

  it('cloudPolygon closes without a repeated point and scales', () => {
  ...
  it('polygons stay inside their box', () => {
    for (const kind of ['rectangle', 'triangle', 'diamond', 'hexagon', 'star', 'cloud']) {
      const pts = geoPolygon(kind as any, 40, 30)
