...
export function strokeOutline(
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (!hasReal && simulate) {
      const d = i
        ? Math.hypot(sp[i * 2] - sp[(i - 1) * 2], sp[i * 2 + 1] - sp[(i - 1) * 2 + 1])
        : 0
  ...
  for (let i = 0; i < n; i++) {
    const x = sp[i * 2],
      y = sp[i * 2 + 1]
    const px = sp[Math.max(0, i - 1) * 2],
      py = sp[Math.max(0, i - 1) * 2 + 1]
    const nx2 = sp[Math.min(n - 1, i + 1) * 2],
      ny2 = sp[Math.min(n - 1, i + 1) * 2 + 1]
    let dx = nx2 - px,
      dy = ny2 - py
...
function dotOutline(x: number, y: number, r: number): number[] {
  const out: number[] = []
