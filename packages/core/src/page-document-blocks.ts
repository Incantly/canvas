...
export function consolidateDocumentBlocks(
  ...
  for (const d of drawings) {
    ...
    merged.height = Math.max(
      merged.height,
      d.height,
      strokeBoundsHeight(merged.strokes),
    );
  }
  return [...base, merged];
}
...
export function appendStrokeToDrawingBlock(
  ...
  return {
    ...
    height: Math.max(
      block.height,
      strokeBoundsHeight(strokes) || DRAWING_BLOCK_MIN_HEIGHT,
    ),
  };
}
...
function drawStrokeInBlock(
  ...
  for (let i = 0; i < stroke.pts.length; i += 3)
    flat.push(stroke.pts[i]!, stroke.pts[i + 1]!);
...
export function parseDocumentBlocksFromDom(
  ...
  for (const child of Array.from(root.children)) {
    if (
      ...
      child.classList.contains("ic-drawing-slot")
    ) {
      const idx = Number(child.dataset.docIndex);
      const prev = existing[idx];
