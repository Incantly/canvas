...
export const Canvas = forwardRef(function Canvas(
  ...
  const {
    theme = 'light', grid = 'lines', readonly = false, hideUi = false,
    themeToggle = true, gridControl = true, watermark = true, store,
    initialSnapshot, snapshot, initialCamera, camera, initialStyles, styles,
    uiTools, uiIcons, hidePagesBar, autoFit = false,
    fitOnMount = autoFit, fitOnResize = autoFit,
    onMount, onChange, onSelectionChange, onThemeChange, onGridChange, onSave,
    className, style,
  } = props
  ...
  useEffect(() => {
    ...
    const ui = buildUI(editor, {
      ...
      onSave: (blob: Blob, background: boolean) => {
        ...
        a.download = 'incantly-' + new Date().toISOString().slice(0, 19).replaceAll(':', '.') + '.png'
        a.click()
    ...
    if (!store && startingSnapshot) editor.store.loadSnapshot(startingSnapshot, 'remote')
  ...
  useEffect(() => { editorRef.current?.setGrid(grid) }, [grid])
  ...
  useEffect(() => {
    uiRef.current?.setOptions({ themeToggle, gridControl, tools: uiTools, icons: uiIcons, hidePagesBar })
